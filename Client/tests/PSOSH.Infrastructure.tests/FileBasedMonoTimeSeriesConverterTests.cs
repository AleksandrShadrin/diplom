...
namespace PSOSH.Infrastructure.tests
{
    ...
    public class FileBasedMonoTimeSeriesConverterTests
    {
        ...
        {
            // arrange
            //string path = "M:\\учеба\\ИТАиОД(Yandex )\\КР\\signal\\Да\\1 — копия — копия — копия — копия.wav";
            ...
            MonoTimeSeries expected = new MonoTimeSeries(arr2, new TimeSeriesInfo("2", "test.wav"));
        ...
        {
            //string path = "M:\\учеба\\ИТАиОД(Yandex )\\КР\\signal\\1\\2.txt";
            if (!File.Exists(path))
            {
        ...
        public void CanConvertCSV()
        {
            ...
            MonoTimeSeries expected = new MonoTimeSeries(arr, new TimeSeriesInfo("3", "w.csv"));
            //CreateElemantInPathCSV(path, arr);
            //if (File.Exists(path))
            //{
            //    File.Delete(path);
            //}
